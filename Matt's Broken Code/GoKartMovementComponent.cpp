...
FVector UGoKartMovementComponent::GetRollingResistance()
{
	...
	//AccelerationDueToGravity is a negative number with UE4 defaults.Therefore, no need to multiply the return value by -1
	return Velocity.GetSafeNormal() * Mass * AccelerationDueToGravity * RollingFrictionCoefficient; //
}

FVector UGoKartMovementComponent::GetVelocity(){return Velocity;}
void UGoKartMovementComponent::SetVelocity(FVector VelocityIn) { Velocity = VelocityIn; }

float UGoKartMovementComponent::GetMaxThrottle(){return MaxThrottle;}

float UGoKartMovementComponent::GetThrottle(){return Throttle;}
void UGoKartMovementComponent::SetThrottle(float ThrottleIn){Throttle = ThrottleIn;}

float UGoKartMovementComponent::GetSteeringThrow(){	return SteeringThrow;}
void UGoKartMovementComponent::SetSteeringThrow(float SteeringThrowIn) {SteeringThrow = SteeringThrowIn;}